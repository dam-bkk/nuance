...
export default function DefMot({ items, allItems, onDone }: { items: VocabItem[]; allItems: VocabItem[]; onDone?: () => void }) {
  ...
  if (questions.length === 0)
    return <p className="text-dim text-center py-12 font-semibold">Aucune définition disponible.</p>;
          ...
          <div className="space-y-3">
            ...
            {mistakes.map((r, i) => (
              <div key={i} className="bg-white rounded-2xl p-4">
                ...
                <p className="text-xs text-dim mt-1 italic">{r.item.definition}</p>
    ...
    <div className="max-w-lg mx-auto space-y-4">
      ...
      <div className="bg-cobalt rounded-4xl p-10 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-lg font-bold text-white leading-relaxed mb-3">{current.item.definition}</p>
          <p className="text-sm text-white/50 italic font-medium">{current.item.traduction}</p>
          <div className="mt-4 flex items-center gap-2">
            <span className="text-xs font-bold text-white/50 bg-white/10 px-2 py-0.5 rounded-full">{current.item.nature}</span>
            <span className="text-xs font-bold text-white/50 bg-white/10 px-2 py-0.5 rounded-full">{current.item.registre}</span>
          </div>
        </div>
      </div>
